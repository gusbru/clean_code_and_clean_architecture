...
test("Should not allow withdrawal with invalid accountId", async () => {
    ...
    await expect(() => assetService.withdraw(input)).rejects.toThrow("Invalid accountId");
});
...
test("Should not allow withdrawal with non-existent account", async () => {
    ...
    await expect(() => assetService.withdraw(input)).rejects.toThrow("Account not found");
});
...
test.each([
    ...
    await expect(() => assetService.withdraw(inputWithdraw)).rejects.toThrow("Invalid assetId");
});
...
test.each([
    ...
    await expect(() => assetService.deposit(input)).rejects.toThrow("Invalid accountId");
});
...
test.each([
    ...
    await expect(() => assetService.deposit(input)).rejects.toThrow("Invalid assetId");
});
...
test.each([
    ...
    await expect(() => assetService.deposit(input)).rejects.toThrow("Invalid quantity");
});